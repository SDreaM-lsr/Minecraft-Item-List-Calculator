...
def main(argv):
    ...
    output_dir = ""
    formula_dir = ""
    program_dir = os.listdir(os.path.dirname(sys.argv[0]))
    ...
    # Default path:
    if not input_dir:
        for for_file in program_dir:
            if for_file[-4:] == ".csv":
                input_dir = os.path.dirname(sys.argv[0]) + "/" + for_file
                break
    if not input_dir:
        print("No csv file existing. Please check and try again.")
        sys.exit(2)
    if not output_dir:
        output_dir = (os.path.dirname(sys.argv[0]) + "/sdm_calculated_" + "material_list-" + local_time + ".csv")
    if not formula_dir:
        for for_file in program_dir:
            if for_file[-5:] == ".json":
                formula_dir = os.path.dirname(sys.argv[0]) + "/" + for_file
                break
    ...
    formula_dir = (os.path.dirname(sys.argv[0]) + "/formula-" + local_time + ".json")
